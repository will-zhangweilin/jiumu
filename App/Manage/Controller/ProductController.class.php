...
class ProductController extends CommonContentController {
	...
	public function addPost() {
		...
		if($id = M('product')->add($data)) {
			...
			$this->success('添加成功',U('Product/index', array('pid' => $pid)));
		}else {
			$this->error('添加失败');
		}
	}
	...
	public function editPost() {
		...
		if (false !== M('product')->save($data)) {
			...
			$this->success('修改成功', U('Product/index', array('pid' => $pid)));
		}else {
	...
	public function move() {
		...
		if (IS_POST) {
			...
			if (false !== M($actionName)->where(array('id'=> array('in', $id)))->setField('cid',$cid)) {
				$this->success('移动成功', U('Product/index', array('pid' => $pid)));
			}else {
				$this->error('移动失败');
			}			
			exit();
	...
	public function delBatch() {
		...
		if (false !== M('product')->where(array('id' => array('in', $idArr)))->setField('status', 1)) {
			...
			$this->success('批量删除成功', U('Product/index', array('pid' => $pid)));
	...
	public function clear() {
		...
		if (M('product')->delete($id)) {
			...
			$this->success('彻底删除成功', U('Product/trach', array('pid' => $pid)));
		}else {
			$this->error('彻底删除失败');
		}
	}
